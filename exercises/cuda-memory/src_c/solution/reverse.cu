...
/* The number of integer elements in the array (declared below) */
...
__global__ void reverseArray(int * d_a, int * d_b)
{
  ...
  tmp[THREADS_PER_BLOCK-(threadIdx.x+1)] = d_a[idx];
  __syncthreads();

  blockOffset = deviceArraySize_ - (blockIdx.x + 1)*blockDim.x;
  d_b[blockOffset + threadIdx.x] = tmp[threadIdx.x];
}
...
int main(int argc, char *argv[])
{
    ...
    int i;
    int hostArraySize = 65536;
    int ncorrect;
    size_t sz;

    sz = hostArraySize * sizeof(int);
    ...
    dim3 blocksPerGrid(hostArraySize/THREADS_PER_BLOCK,1,1);
    ...
    ncorrect = 0;
    for (i = 0; i < hostArraySize; i++) {
      if (h_out[i] == h_in[hostArraySize - (i+1)]) ncorrect += 1;
    }
    printf("Number of correct elements %d (%s)\n", ncorrect,
           ncorrect == hostArraySize ? "Correct" : "INCORRECT");
    printf("\n");
