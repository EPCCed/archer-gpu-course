...
int main(int argc, char *argv[]) {
  ...
  /* Execute the kernel by means of a graph */
  {
    ...
    /* Add cudaMemcpyDeviceToHost as a dependent */

    cudaGraphNode_t mNode = {0};
    cudaMemcpy3DParms mParams = {0};

    mParams.kind = cudaMemcpyDeviceToHost;
    mParams.extent = make_cudaExtent(ncol*mrow*sizeof(double), 1, 1);
    mParams.srcPos = make_cudaPos(0, 0, 0);
    mParams.srcPtr = make_cudaPitchedPtr(d_a, mrow*ncol*sizeof(double),
                                         mrow*ncol, 1);
    mParams.dstPos = make_cudaPos(0, 0, 0);
    mParams.dstPtr = make_cudaPitchedPtr(h_a, mrow*ncol*sizeof(double),
                                         mrow*ncol, 1);

    cudaGraphAddMemcpyNode(&mNode, graph, &kNode, 1, &mParams);

    /* Instantiate and run the graph in default (NULL) stream */

    CUDA_ASSERT( cudaGraphInstantiate(&graphExec, graph, NULL, NULL, 0) );

    CUDA_ASSERT( cudaGraphLaunch(graphExec, NULL) );
